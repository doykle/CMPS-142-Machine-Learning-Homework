...
def evaluate( clf, dumb_clf, instances, labels ):
   ...
   # Baseline where everyone is passing
   for idx,val in enumerate( prediction_array ):
      prediction_array[idx] = 'pass'

   matrix_all_pass = confusion_matrix( labels, prediction_array )

   all_pass_accuracy = 1.0 - ( float( matrix_all_pass[0][1] ) / ( matrix_all_pass[0][1] + matrix_all_pass[1][1] ) )
   print( "\nAll passing accuracy: ", all_pass_accuracy )
